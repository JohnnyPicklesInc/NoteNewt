...
export async function onRequestGet({ request, env }) {
  ...
  const user = await env.DB.prepare('SELECT label, recovery_lookup FROM users WHERE id = ?').bind(me.userId).first();
  ...
  return json({ userId: me.userId, label: user.label, passkeys: count?.n || 0, hasRecovery: !!user.recovery_lookup });
}
